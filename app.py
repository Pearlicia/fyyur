...
app.config['SQLALCHEMY_DATABASE_URI']='postgresql://postgres:pearlicia@localhost:5432/fyyur'
...
@app.route('/home')
def index():
    # Used id to sort them since it's serial!
    try:
...
def venues():
    ...
    try:
        cities_states = venue.query.with_entities(venue.city, venue.state).group_by(venue.city, venue.state).all() 
        venues = [] 
        venue_data = [] 
